...
**Audit chain is the trust moat.** Hash-chained JSONL, ed25519-signed with the `init` keypair, hashes-only at write (redaction by construction). `--verify` re-walks seq/prev_hash/signatures; `--export` produces a signed content-addressed bundle. Key deletion reads BROKEN, not unsigned. This is the artifact you hand to compliance — and OpenCode/Claude can't ship it without repudiating their business model.
