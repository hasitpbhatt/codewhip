...
- [x] **Claude Code parity program — waves 1–2 of 5 closed, wave 3 open.** Row-level build order
  ...
  putting the loop's own decisions on a channel, and `.context`/`.usage`/`.cost`
  giving the session a ledger: 43 HAVE/ALIAS,
  15 PARTIAL, 44 GAP (**42.2%**), 22 rows left in the wave, led by parallel tool
