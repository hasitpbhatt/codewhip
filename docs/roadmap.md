...
governance. H2 = credible open alternative to the incumbent closed-trust model.
...
- [x] **`--debug` / `--debug-file`** — the loop's own decisions on a channel: which consent-ladder rung answered, why rotation was skipped, transcript weight at each metering point, hook outcomes, per-call exec timing. Redaction enforced at the sink (one place, so a call site cannot forget or opt out); `*.env*` and any `.codewhip/` path refused at parse time; 8 MiB cap and any write error warn once then go quiet rather than killing the run; synchronous append so a crashed run keeps its tail; subagents inherit the parent's sink. Shipped 2026-09-25 as parity wave 3b (`src/debug.ts`, 11 tests). No prompt cost — receipts unchanged. *(shipped 2026-09-25)*
- [x] **`.context` / `.usage` / `.cost`** — the session keeps a ledger now: the loop reports the shape it actually sent (system prompt, tool specs, the compaction ceiling it used) and each run's usage folds into per-`provider:model` totals. `.context` prints the grid — fixed costs, transcript by role, used/free against the ceiling; `.usage` the token totals with `(est.)` on any leg the provider did not meter; `.cost` the dollars, and *refuses* a total when a leg has no price row rather than rendering `$0.00`. Estimates are labelled as estimates in the header itself, and the whole thing is in-memory until `.exit` (no files, no prompt tokens). Shipped 2026-09-25 as parity wave 3c — a PARTIAL row, so the GAP pile is untouched (`src/session-ledger.ts`, 15 tests). *(shipped 2026-09-25)*
- [x] **Parallel tool execution** — a model turn that returns several tool calls now executes the provably-safe ones concurrently instead of one `for` loop at a time (`src/parallel-tools.ts`, the eligibility scan and ordered commit at `src/loop.ts:813`–`:932`). The gate is deliberate and fail-safe: the whole turn stays serial the moment one call cannot be *proven* free of workspace or harness-state writes, would need a keystroke, or has no pre-resolved grant — so no read races a half-applied edit and two approvals never contend for one terminal. Eligible set is `read` + pre-approved `webfetch`, in flight capped at 4. Ordering is not negotiable: each call still runs the real policy gates and keeps the ruleId/actor the ladder would have written (`allow:default:webfetch:ask+yolo` for yolo), and `seq`, the `tool_result` messages, the trace, `outcomes.jsonl` and the hash chain all commit in assistant order rather than completion order. Each call keeps its own timeout and catch, so one crash is one failed result, not a failed batch. Shipped 2026-09-25 as parity wave 3g (`src/parallel-tools.test.ts`, 6 tests) — the row closed GAP → HAVE, GAP 40 → 39, wave 3 18 → 17. No new dependency, no prompt cost, no schema change. *(shipped 2026-09-25)*
- [x] **Subagent frontmatter: `tools` + `disallowedTools`** — an agent file can now say what its child may touch. `tools:` narrows a child's advertised set and cannot widen it (`bash` in a `tools:` list is a parse error, not a promise the harness breaks), `disallowedTools:` speaks the same grammar as `--disallowed-tools`, and both compile onto the ONE refusal list the child is born with — so un-advertising a spec and refusing a call stay one mechanism. The parent's refusals cross the delegation boundary now too, and the eight fields this harness cannot honour (`permissionMode`, `skills`, `mcpServers`, `hooks`, `memory`, `background`, `effort`, `isolation`) are refused BY NAME with their reason, as is any unknown key: a field that silently does nothing is the failure mode worth more than a load error. `codewhip agents` shows `tools=read` and prints the skipped-file line. Shipped 2026-09-25 as parity wave 3d — the row moved GAP → PARTIAL (not HAVE: 2 of 10 fields are honoured, 8 are refused), GAP 44 → 43 (`src/subagents.ts`, 9 tests). Cost: a narrower child prompt; nothing added to any system prompt. *(shipped 2026-09-25)*
- [x] **`ask_user` — the model can ask a question back** — the thirteenth builtin: one multiple-choice question (2-6 distinct options, optional `multiSelect`) put to the human who started the run, with their answer arriving as an ordinary tool result. It grants nothing: allow-class at the ladder, because its whole effect is one keystroke — every mutation the model goes on to attempt still walks the consent check — and grading the *question* as an ask would be circular. Still deniable, since its subject is the question text (`deny ask_user:Revert the branch?*` in `policy.md`). It exists only where a human is: the channel is injected when the CLI owns a readline prompt on a TTY, and headless `-p`, an SDK `query()` and a TUI run get no spec at all — never advertise what the harness refuses. A child cannot hold the keyboard (`loop:child-no-prompt`, recorded on the child's own runId), and no answer is reported as a refusal that echoes the unanswered question, so silence cannot be read as assent. Free text outranks the menu: a number in range picks an option, anything else is the human's own words verbatim. Shipped 2026-09-25 as parity wave 3e — the row moved GAP → PARTIAL (not HAVE: no four-questions-per-call, no `header`, no `preview`), GAP 43 → 42 (`src/tools/ask-user.ts`, 15 tests). Cost: 843 chars of extra schema per provider call in interactive runs; headless and SDK send nothing, and no prompt text was added. *(shipped 2026-09-25)*
- [x] **`--agents` JSON roster + `--agent` on the main thread** — the file roster without the file. `--agents '{"name":{…}}'` compiles through the same parser and the same refusals as `.codewhip/agents/*.md` (`src/subagents.ts:209`), so a flag cannot be laxer than a commit — and precedence is built-ins → files → flag, visible to `delegate` as well as to `codewhip agents`, so one name means one agent. `--agent <name>` runs *as* an entry: its prompt is **appended** to the harness base rather than replacing it (the base is what tells the model which calls get refused), its `model` applies only over an unset `--model` and turns rotation off, and its `maxTurns` bounds children only — a human's run keeps the budget the human set. An entry that cannot be honoured stops the run at parse time, and the run prints what the persona costs (`!! --agent auditor: its 10-char prompt joins the system prompt and re-pays every turn…`) while sharing the existing 8 000-char append budget so the cap errors instead of slicing. Shipped 2026-09-25 as parity wave 3f — the row moved GAP → PARTIAL (not HAVE: append-not-replace, child-only `tools` narrowing, no mid-session switch), GAP 42 → 41 (`src/subagents.ts`, `src/agents-flag.test.ts`, 14 tests). Cost: prompt text in the run that asks for it; nothing elsewhere. *(shipped 2026-09-25)*
- [x] **Hook lifecycle events** — the event surface goes 3 → 11 of 33: `SessionStart`/`UserPromptSubmit` before the first turn, `PreCompact`/`PostCompact` around compaction, `SubagentStart`/`Stop` around delegation, `StopFailure`/`SessionEnd` on exit (`src/hooks.ts`, `fireLifecycle` at `src/loop.ts:535`). The durable decision is the `hookPolicy` table (`src/hooks.ts:103`): what a hook may ASK FOR is a property of the EVENT, not of which field it printed — so an observe-only seam refuses `continue:false` and injected context by rule, and says so out loud instead of failing silently. Only pre-turn seams may stop a run (`PreToolUse`, `SessionStart`, `UserPromptSubmit`; pinned by a test), and a `SessionStart` veto skips the turn loop so the receipt honestly reads zero tokens. Lifecycle events take `match: "*"` only — a tool name there is a load error naming why — while `Stop` keeps its match-agnostic dispatch but still validates strictly. Every seam is awaited (an un-awaited hook outliving the run is an orphan) and inherits redaction. Shipped 2026-09-25 as parity wave 3h — the row moved GAP → PARTIAL (not HAVE: 22 events still absent and named), GAP 39 → 38, wave 3 17 → 16 (`src/loop-hooks.test.ts`, 6 tests). No new dependency, no prompt cost, no schema change. *(shipped 2026-09-25)*
- [x] **Custom slash commands** — `.codewhip/commands/*.md` + `$ARGUMENTS`, REPL `.help` + error-on-unknown, one-shot exact-match expansion. Ruling: `docs/moat/19-qol-parity.md`. *(shipped 2026-09-21)*- [x] **Hooks** — `PreToolUse/PostToolUse/Stop` from self-protected `hooks.json`, fail-open-on-infra-failure, redaction-downstream, observe-only Stop. Ruling: `docs/moat/19-qol-parity.md`. *(shipped 2026-09-21)*
- [x] **Terminal parity program — waves 1–2 of 5 closed, wave 3 open.** Row-level build order
  lives in `docs/moat/20-parity-matrix.md` (the matrix counts itself:
  ...
  with per-item tasks whose blockers gate claiming, `--debug`/`--debug-file`
  putting the loop's own decisions on a channel, `.context`/`.usage`/`.cost`
  giving the session a ledger, agent files gaining `tools`/`disallowedTools`
  (the other eight fields refused by name), `ask_user` letting the model put
  one question back to the human, and `--agents`/`--agent` moving that roster onto
  the command line: 43 HAVE/ALIAS,
  18 PARTIAL, 41 GAP (**42.2%**), 19 rows left in the wave, led by parallel tool
