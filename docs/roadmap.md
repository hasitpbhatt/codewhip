...
- [x] **Claude Code parity program — waves 1–2 of 5 closed, wave 3 open.** Row-level build order
  ...
  with per-item tasks whose blockers gate claiming, and `--debug`/`--debug-file`
  putting the loop's own decisions on a channel: 42 HAVE/ALIAS,
  16 PARTIAL, 44 GAP (**41.2%**), 22 rows left in the wave, led by parallel tool
