...
- [ ] **Todo tool** — `todo` list/replace/update over `.codewhip/todos.json`, harness-state allow (`default:todo:allow`), child-denied, redacted at save. Ruling: `docs/moat/19-qol-parity.md`.
- [ ] **Custom slash commands** — `.codewhip/commands/*.md` + `$ARGUMENTS`, REPL `.help` + error-on-unknown, one-shot exact-match expansion. Ruling: `docs/moat/19-qol-parity.md`.
- [ ] **Hooks** — `PreToolUse/PostToolUse/Stop` from self-protected `hooks.json`, fail-open-on-infra-failure, redaction-downstream, observe-only Stop. Ruling: `docs/moat/19-qol-parity.md`.
