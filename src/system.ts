...
export const SYSTEM_PROMPT = [
  ...
  "Multi-step work: open with todo replace (the full plan, one in_progress at a time, blocks/blockedBy on the items that genuinely depend on each other), update statuses as steps finish, list to re-read, get an id before claiming someone else's item. A blocked item cannot be claimed until its blockers are done — the order is enforced, so draw the edges honestly. The list is harness state, not a deliverable — it keeps you (and the user) oriented, nothing more.",
